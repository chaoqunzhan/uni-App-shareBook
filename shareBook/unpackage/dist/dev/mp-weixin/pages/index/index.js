...
var _default =
{
  ...
  methods: {
    ...
    waterfall: function waterfall() {var _this = this;
      // 用promise获取数据
      var that = this;
      ...
      promise.then(function (res) {
        ...
        _this.cardList = res;
        _this.cardListLeft.push(_this.cardList[0]);
        _this.preLoadImg = _this.cardList[0].image[0];
        var that = _this;
      ...
      if (this.loadMoreTemp == 1) {//loadMoreTemp==1,才允许触发
        ...
        promise.then(function (res) {
          ...
          if (!newcardList.length == 0) {//判断是否还有新数据
            _this2.cardList = _this2.cardList.concat(newcardList); //返回的新数据加到当前的cardList
            if (_this2.cardLeftHeight > _this2.cardRightHeight) {//把第一个新数据加到目前更低的栏上，以触发@load="onImageLoad"
              _this2.cardListRight.push(newcardList[0]);
            } else {
              _this2.cardListLeft.push(newcardList[0]);
            }
          } else {
            _this2.showNoMore = true; //没有新数据就显示到底了
          }
        });
